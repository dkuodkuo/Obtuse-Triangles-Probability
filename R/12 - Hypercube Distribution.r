...
for (i in 1:16){
  point1 = matrix(runif(m*dimensions[i]), ncol = dimensions[i])
  point2 = matrix(runif(m*dimensions[i]), ncol = dimensions[i])
  point3 = matrix(runif(m*dimensions[i]), ncol = dimensions[i])
...
plot2 = ggplot() + 
  ...
  coord_cartesian(xlim = c(-2, 17.5), ylim = c(-0.15, 0.75)) +
  guides(color=FALSE) +
